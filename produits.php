<?php
	...
	function getProduct($id=0)
	{
		...
		if($id != 0)
		{
			$query .= " WHERE id_produit=".$id." LIMIT 1";
		}
		$response = array();
		$result = mysqli_query($conn, $query);
		while($row = mysqli_fetch_array($result))
	...
	function AddProduct()
	{
		...
		echo $query="INSERT INTO produit(name, description) VALUES('".$name."', '".$description."')";
	...
	function updateProduct($id)
	{
		...
		$modified = date('Y-m-d H:i:s');
		$query="UPDATE produit SET name='".$name."', description='".$description."', modified='".$modified."' WHERE id=".$id;
